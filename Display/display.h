 ...
 *
 *  Версия: 1.4 Turbo (CMSIS, LL, assembler) для AT32F403A
 *
...
//подсветка
typedef struct {
	tmr_type *htim_bk;		//------- для подсветки с PWM:- таймер
	uint32_t channel_htim_bk;	//----------------------------- канал таймера

	gpio_type *blk_port;		//просто для включения и выключения подсветки, если htim_bk = 0 (без PWM, определен порт вывода)
...
extern void Display_TC_Callback(dma_type*, uint32_t);
